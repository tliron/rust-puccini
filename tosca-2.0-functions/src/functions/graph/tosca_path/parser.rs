...
impl<'own> ToscaPathParser<'own> {
        ...
        match argument {
            Any::Text(text) => match text.as_str() {
                ...
                "SELF" => match site.id.kind {
                    Kind::Node => Ok(self.next_site_after_node(&site)?.unwrap_or_else(|| site)),

                    Kind::Relationship => Ok(self.next_site_after_relationship(&site)?.unwrap_or_else(|| site)),
    ...
    pub fn next_any(&mut self, property: &Property) -> Result<Any, String> {
        let mut current_value = property.get_value()?;
