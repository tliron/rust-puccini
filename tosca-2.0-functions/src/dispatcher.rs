...
impl Guest for Dispatcher {
    ...
    fn dispatch(name: String, arguments: Vec<Any>, site: Site) -> Result<Any, Error> {
        match &*name {
            ...
            "tosca:2.0:_list" => list(&arguments, &site),
            "tosca:2.0:_map" => map(&arguments, &site),
            "tosca:2.0:_timestamp" => timestamp(&arguments, &site),
            "tosca:2.0:_scalar" => scalar(&arguments, &site),
            "tosca:2.0:_version" => version(&arguments, &site),
