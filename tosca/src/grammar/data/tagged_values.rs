        ...
        ErrorRecipientT: ErrorRecipient<ResolveError<AnnotatedT>>,
    {
        ...
        match self {
            Self::List(list) => {
                for item in list {
                    match item {
                        Self::Map(map) => match map.to_key_value_pair() {
                            Some(pair) => {
                            ...
                            None => errors.give(
                                MalformedError::new("tag-value pair".into(), "is not a map with a single key".into())
                                    .with_annotations_from(item),
                            )?,
                        },

                        _ => errors.give(IncompatibleVariantTypeError::new(item, &["map"]))?,
                    }
                }
            }

            _ => errors.give(IncompatibleVariantTypeError::new(self, &["list"]))?,
        }
