...
use {
    ...
    duplicate::*,
    kutil::{std::error::*, unwrap_or_give, unwrap_or_give_and_return},
    read_url::*,
    std::io,
};
        ...
        ErrorRecipientT: ErrorRecipient<ToscaError<AnnotatedT>>,
    {
        ...
        let (variant, url_context) = match source_id {
            SourceID::UUID(_) => {
                ...
                (unwrap_or_give_and_return!(parser.parse(&mut io::stdin()), errors, Ok(())), url_context.clone())
            }
            ...
            SourceID::URL(url) => {
                ...
                (
                    {
                        let parser = Parser::new(Format::YAML).with_source(source_id.into());
                        unwrap_or_give_and_return!(parser.parse(&mut reader), errors, Ok(()))
                    },
                    url.base()
                        .and_then(|base| {
        ...
        Ok(match get_dialect_id(&variant).cloned() {
            Some(dialect_id) => {
                ...
                // Merge namespaces
                for (source_id, scope) in dependencies {
                    let dependency = self.get_source(&source_id)?;
