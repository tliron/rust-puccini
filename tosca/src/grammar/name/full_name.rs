...
impl FullName {
    ...
    /// To Floria group ID.
    pub fn to_floria_group_id(&self, prefix: &str) -> floria::ID {
        let (mut floria_scope, name) = (self.scope.to_floria_scope(), self.name.clone());
        floria_scope.insert(0, prefix.into());
        floria_scope.insert(0, "tosca".into());
        floria::ID::new_for(floria::Kind::Group, floria_scope, name.into())
    }
}

impl Debuggable for FullName {
    fn write_debug_for<WriteT>(&self, writer: &mut WriteT, context: &DebugContext) -> io::Result<()>
    where
        WriteT: io::Write,
    {
        context.separate(writer)?;
        self.scope.write_debug_for(writer, context)?;
        self.name.write_debug_for(writer, &context.clone().with_separator(false))
    }
}
