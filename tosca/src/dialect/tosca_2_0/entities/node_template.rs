    ...
    AnnotatedT: Annotated + Clone + Default,
{
        ...
        ErrorRecipientT: ErrorRecipient<ToscaError<AnnotatedT>>,
    {
        let mut floria_node_template =
            floria::NodeTemplate::new_for(context.floria_scope.clone(), node_template_name.into(), None);
    ...
    AnnotatedT: 'static + Annotated + Clone + Default,
{
    ...
    fn complete(
        ...
        if let Some(copy) = &self.copy {
            ...
            if self.directives.is_empty() && !copy.directives.is_empty() {
                self.directives = copy.directives.clone();
            }
            ...
            if self.requirements.is_empty() && !copy.requirements.is_empty() {
                self.requirements = copy.requirements.clone();
            }
            if self.capabilities.is_empty() && !copy.capabilities.is_empty() {
                self.capabilities = copy.capabilities.clone();
            }
            if self.interfaces.is_empty() && !copy.interfaces.is_empty() {
                self.interfaces = copy.interfaces.clone();
            }
            if self.artifacts.is_empty() && !copy.artifacts.is_empty() {
                self.artifacts = copy.artifacts.clone();
            }
            if_none_clone(&mut self.count, &copy.count);
            if_none_clone(&mut self.node_filter, &copy.node_filter);
        }
