use super::{
    super::{
        super::super::grammar::{File as CommonFile, *},
        catalog::OldCatalog,
    },
    artifact_type::*,
    capability_type::*,
    ...
    AnnotatedT: Annotated + Clone + Default,
{
    ...
    #[resolve]
    #[debuggable(iter(kv), as(debuggable), key_style(string))]
    pub policy_types: PolicyTypeMap<AnnotatedT>,
...
impl<'own, AnnotatedT> CommonFile<OldCatalog<'own, AnnotatedT>, AnnotatedT> for File<AnnotatedT>
where
    AnnotatedT: Annotated + Clone + Default,
{
    fn get_importables(&self) -> Vec<Importable> {
        let mut importables = Vec::with_capacity(self.imports.len());

        for import in &self.imports {
            if let Some(url) = &import.url {
                importables.push(Importable::new(url.clone(), import.namespace.clone().map(|name| name.into())));
            }
        }
    ...
    {
        let mut floria_group = floria::Group::new_for(context.floria_scope.clone(), source);
        floria_group.metadata.set_tosca_entity("File");
        floria_group.metadata.set_tosca_description(self.description.as_ref());
        floria_group.metadata.merge_tosca_metadata(&self.metadata);

        let floria_group_id = floria_group.id.clone();
        if let Err(error) = context.store.add_group(floria_group) {
            errors.give(error)?;
            return Ok(None);
        }

        if let Some(service_template) = &self.service_template {
            service_template.compile_to_floria(context.clone(), &floria_group_id, errors)
        } else {
            Ok(None)
        }
    }
