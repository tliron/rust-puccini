...
#[derive(Clone, Debug, Debuggable, Resolve, SmartDefault)]
    ...
    AnnotatedT: Annotated + Clone + Default,
{
    ...
    #[resolve]
    #[debuggable(iter(item), style(symbol), tag = tag::span)]
    ...
    AnnotatedT: Annotated + Clone + Default,
{
        ...
        ErrorRecipientT: ErrorRecipient<ToscaError<AnnotatedT>>,
    {
        ...
        let mut floria_namespace = context.floria_scope.clone();
    ...
    AnnotatedT: 'static + Annotated + Clone + Default,
{
    fn complete(
        &mut self,
        parent: &RequirementDefinition<AnnotatedT>,
        depot: &mut Depot,
        source_id: &SourceID,
        scope: &Scope,
        errors: ToscaErrorRecipientRef,
    ) -> Result<(), ToscaError<WithAnnotations>> {
        let _errors = &mut errors.to_error_recipient();

        // TODO: validate node (template) adheres to parent's node (type)

        // TODO: validate capability adheres to parent's capbility type

        match &mut self.relationship {
            Some(relationship) => {
                relationship.complete(&parent.relationship, depot, source_id, scope, errors)?;
            }

            None => self.relationship = Some(parent.relationship.into_scoped(scope)),
        }
...
where
    AnnotatedT: Annotated + Clone + Default,
{
    fn into_scoped(&self, scope: &Scope) -> RequirementAssignment<AnnotatedT> {
        RequirementAssignment {
...
impl<'own, AnnotatedT> Assignment<RequirementDefinition<AnnotatedT>, OldCatalog<'own, AnnotatedT>, AnnotatedT>
    for RequirementAssignment<AnnotatedT>
where
    AnnotatedT: Annotated + Clone + Default,
{
    ...
    fn complete<ErrorRecipientT>(
        &self,
        context: AssignmentCompleteContext<'_, RequirementDefinition<AnnotatedT>, OldCatalog<'own, AnnotatedT>>,
        errors: &mut ErrorRecipientT,
        ...
        ErrorRecipientT: ErrorRecipient<ToscaError<AnnotatedT>>,
    {
        let relationship = match &self.relationship {
            Some(relationship) => Some(relationship.complete(
                AssignmentCompleteContext {
                    name: "",
                    definition: &context.definition.relationship,
                    catalog: context.catalog,
                },
                errors,
            )?),

            None => Some(RelationshipAssignment::from_definition(
                AssignmentFromDefinitionContext {
                    name: "",
                    definition: &context.definition.relationship,
                    catalog: context.catalog,
                },
        ...
        Ok(Self {
            node: match &self.node {
                // TODO: refine, node template matches node types?
                Some(node) => Some(node.clone()),
                None => context.definition.node.as_ref().map(|name| IndexedFullName::new(name.clone(), None)),
            },
            capability: match &self.capability {
                Some(capability) => Some(capability.clone()),
                None => Some(context.definition.capability.clone()),
            },
            relationship,
            allocation: None,
            count: context.definition.count_range.lower,
            node_filter: context.definition.node_filter.clone(),
    ...
    fn from_definition<ErrorRecipientT>(
        context: AssignmentFromDefinitionContext<'_, RequirementDefinition<AnnotatedT>, OldCatalog<'own, AnnotatedT>>,
        errors: &mut ErrorRecipientT,
    ) -> Result<Self, ToscaError<AnnotatedT>>
    where
        ErrorRecipientT: ErrorRecipient<ToscaError<AnnotatedT>>,
    {
        Ok(Self {
            node: context.definition.node.as_ref().map(|node| IndexedFullName::new(node.clone(), None)),
            capability: Some(context.definition.capability.clone()),
            relationship: Some(RelationshipAssignment::from_definition(
                AssignmentFromDefinitionContext {
                    name: "",
                    definition: &context.definition.relationship,
                    catalog: context.catalog,
