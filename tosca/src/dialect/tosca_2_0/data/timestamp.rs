...
impl FromStr for Timestamp {
    ...
    fn from_str(representation: &str) -> Result<Self, Self::Err> {
        ...
        let datetime = DateTime::parse_from_rfc3339(representation).map_err(|_error| "not RFC 3339")?;

        Ok(Self { datetime })
    }
}

impl<AnnotatedT> Into<Variant<AnnotatedT>> for Timestamp
where
    AnnotatedT: Default,
{
