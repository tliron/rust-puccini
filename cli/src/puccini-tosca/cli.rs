...
pub struct Compile {
    ...
    /// simulate instantiation into Floria prefix;
    /// incompatible with `--parse-only`
    #[arg(long = "instantiate", short = 'i', verbatim_doc_comment)]
    pub instantiate: bool,

    /// disable completion and compilation phases;
    /// useful for debugging; incompatible with `--instantiate`
    #[arg(long = "parse-only", verbatim_doc_comment)]
    pub parse_only: bool,
